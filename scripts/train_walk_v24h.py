"""Walk-only training v24h: reward normalization only, NO obs normalization.
Insight: obs normalization creates train/eval mismatch (v24d-f failed).
         No normalization kills value function (v24g explained_variance=0.001).
         Reward normalization helps value function without affecting eval.
"""
import sys, os
...
print(f"Training {MODE}-only v24h, {N_ENVS} envs, {TOTAL_STEPS:,} steps...")
...
eval_callback = EvalCallback(
    eval_env, best_model_save_path="checkpoints/walk_v24h_best/",
    eval_freq=50_000, n_eval_episodes=10, deterministic=True, verbose=1,
)

model.learn(total_timesteps=TOTAL_STEPS, callback=eval_callback)

model.save("checkpoints/walk_v24h")
print("Saved to checkpoints/walk_v24h")

# Evaluate — no obs normalization needed!
