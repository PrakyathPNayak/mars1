...
class MiniCheetahEnv(gym.Env):
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        if mode in ("walk", "run"):
            ...
            # v31j: Overshoot penalties — penalize going faster than commanded.
            # v31k: Added 20% dead zone. Without it, EMA noise at correct speed
            # triggered penalties, teaching model "safer to not move at all."
            # walk_fwd went from vx=1.35 (v31i) to vx=0.08 (v31j 1200K) — DEAD.
            _OVERSHOOT_DEADZONE = 1.20  # only penalize when >120% of commanded
            if abs(vx_cmd) > 0.05:
                r_vx_overshoot = max(0.0, abs(vx_ema) - _OVERSHOOT_DEADZONE * abs(vx_cmd))
            else:
                r_vx_overshoot = 0.0
            if abs(vy_cmd) > 0.05:
                r_vy_overshoot = max(0.0, abs(vy_ema) - _OVERSHOOT_DEADZONE * abs(vy_cmd))
            else:
                r_vy_overshoot = 0.0
