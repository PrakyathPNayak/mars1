...
class MiniCheetahEnv(gym.Env):
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        if mode in ("walk", "run"):
            ...
            if mode == "run":
                ...
                total = (
                    14.0 * r_vx_track_walk   # v31s6g5: boosted 8→14 — fwd_yaw fix (vx was 11 vs wz 27)
                    + 4.0 * r_vx_lin         # v31s6g5: boosted 3→4
                    + 3.0 * r_vy_track       # EMA-based lateral tracking
                    + 2.0 * r_vy_lin         # monotonic lateral gradient
                    + 12.0 * r_wz_track      # yaw tracking (unchanged)
                    + 8.0 * r_wz_lin         # v31s6g5: reduced 15→8 — fwd_yaw fix (now vx=18, wz=20)
                    ...
                    - 6.0 * r_wz_overshoot   # v31s6g5: boosted -2→-6, compensate for reduced wz_lin
    ...
    def _randomize_command_for_mode(self, rng):
        ...
        if mode == "stand":
            ...
            # v31s6g5: Rebalanced sampling to fix walk_back (24%) and lat_L (67%) regression.
            # Added dedicated backward category. Widened yaw range to 1.2.
            ...
            if roll < 0.25:
                ...
                # v31s6g5: Dedicated backward (was merged with forward, got only ~10%)
                vx = float(rng.uniform(-0.5, -0.1))
                vy = float(rng.uniform(-0.05, 0.05))
                wz = float(rng.uniform(-0.05, 0.05))
            elif roll < 0.48:
                ...
                wz = float(rng.uniform(-0.8, 0.8))
                if abs(vy) < 0.15 and abs(wz) < 0.15:
                    vy = float(rng.choice([-0.4, 0.4]))
            elif roll < 0.75:
                # Pure yaw with wider range (was ±1.0, now ±1.2)
                vx = float(rng.uniform(-0.05, 0.05))
                vy = float(rng.uniform(-0.05, 0.05))
                wz = float(rng.uniform(-1.2, 1.2))
                if abs(wz) < 0.2:
                    wz = float(rng.choice([-0.8, 0.8]))
            elif roll < 0.85:
                # v31s6g6: Dedicated fwd+yaw — fixes -15% fwd_yaw tracking
                # Ensures model sees combined forward+turn commands frequently
                vx = float(rng.uniform(0.3, 0.8))
                vy = float(rng.uniform(-0.05, 0.05))
                wz_sign = float(rng.choice([-1.0, 1.0]))
                wz = wz_sign * float(rng.uniform(0.3, 0.8))
            else:
                # General mixed commands (reduced 25%→15% to fit fwd+yaw)
            ...
            # v31s6g5: Extended range to enable 2.0+ m/s. Curriculum:
            # 40% easy [0.3,1.0], 40% medium [1.0,2.0], 20% stretch [2.0,3.0]
            r = rng.random()
            if r < 0.4:
