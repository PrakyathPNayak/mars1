...
class MiniCheetahEnv(gym.Env):
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        if mode in ("walk", "run"):
            ...
            # Without this, r_vx_lin gives flat +2.0 even at vx=1.6 for cmd=0.5,
            # and tracking exp() has zero gradient far from target.
            # Robot learned to sprint at 1.6+ and crash instead of walking at 0.5.
            if abs(vx_cmd) > 0.05:
                r_vx_overshoot = max(0.0, abs(vx_ema) - abs(vx_cmd))
            else:
                r_vx_overshoot = 0.0
            if abs(vy_cmd) > 0.05:
                r_vy_overshoot = max(0.0, abs(vy_ema) - abs(vy_cmd))
            else:
                r_vy_overshoot = 0.0
            ...
            if mode == "run":
                ...
                total = (
                    ...
                    - 3.0 * r_vx_overshoot   # v31j: penalize exceeding commanded fwd speed
                    - 2.0 * r_vy_overshoot   # v31j: penalize exceeding commanded lat speed
                )
                scaled_components = {
                    ...
                    "r_vy_overshoot": -2.0 * r_vy_overshoot,
                    "r_vx_ema": vx_ema,
                    "r_total": total,
                }
        elif mode == "jump":
