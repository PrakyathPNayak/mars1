...
class MiniCheetahEnv(gym.Env):
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        if mode in ("walk", "run"):
            ...
            # Quadratic was too weak for small drift: 0.1²=0.01 → penalty 0.04
            # Linear: 0.1 → penalty 0.4 (10× stronger at small drift)
            r_vx_unwanted = abs(vx_ema) if abs(vx_cmd) < 0.05 else 0.0
            r_vy_unwanted = abs(vy_ema_val) if abs(vy_cmd) < 0.05 else 0.0
            r_wz_unwanted = abs(wz_ema_val) if abs(wz_cmd) < 0.05 else 0.0
            ...
            if mode == "run":
                ...
                total = (
                    ...
                    - 2.0 * r_wz_unwanted    # v31f: LINEAR anti-spin
                    - 1.0 * r_heading_drift  # v31f: always-on heading stabilizer
                )
                scaled_components = {
                ...
                r_height_walk = math.exp(-(effective_height - self._effective_target_height)**2 / walk_height_sigma)
                ...
                total = (
                    ...
                    - 4.0 * r_wz_unwanted    # v31f: LINEAR anti-spin
                    - 1.5 * r_heading_drift  # v31f: always-on heading stabilizer
                )
                scaled_components = {
                    ...
                    "r_heading_drift": -1.5 * r_heading_drift,
                    "r_vx_ema": vx_ema,
                    "r_total": total,
                }
        elif mode == "jump":
