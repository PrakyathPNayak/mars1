...
class MiniCheetahEnv(gym.Env):
    ...
    def _compute_walk_reference_action(self) -> np.ndarray:
        ...
        # v31d: Mode-dependent gait amplitude
        # Walk base=0.32 (normalized at 0.5 m/s), Run base=0.45 (normalized at 2.0 m/s)
        # BUG FIX: v31b/c had extra 0.30 multiplier → amp_hip=0.096 for walk_fwd
        # This made forward reference WEAKER than lateral lat_boost=0.15!
        is_run = self.command_mode == "run"
        vx_norm = 2.0 if is_run else 0.5
        base_amp = 0.45 if is_run else 0.32
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        if mode in ("walk", "run"):
            ...
            # v31n: Overshoot penalties — use INSTANTANEOUS velocity (not EMA).
            # Bug found: EMA lags ~20 steps behind actual velocity. Model exploits
            # this by sprinting (vx=1.3 for cmd=0.5) while EMA is still near target.
            # Result: high tracking + zero overshoot for 20 steps = free sprint window.
            # Fix: instantaneous vx catches sprints IMMEDIATELY. 20% dead zone absorbs
            # natural gait oscillation (±0.1 at cmd=0.5 → peak 0.6 = dead zone edge).
            _OVERSHOOT_DEADZONE = 1.20  # only penalize when >120% of commanded
            if abs(vx_cmd) > 0.05:
                r_vx_overshoot = max(0.0, abs(vx) - _OVERSHOOT_DEADZONE * abs(vx_cmd))
            else:
                r_vx_overshoot = 0.0
            if abs(vy_cmd) > 0.05:
                r_vy_overshoot = max(0.0, abs(vy) - _OVERSHOOT_DEADZONE * abs(vy_cmd))
            else:
                r_vy_overshoot = 0.0
    ...
    def _randomize_command_for_mode(self, rng):
        ...
        if mode == "stand":
            ...
            if roll < 0.35:
                ...
                # v31k: Extended backward range from -0.3 to -0.5 (was dead at cmd=-0.5)
