...
class TestSuite:
    ...
    def _test_policy_components(self):
        ...
        obs_hist = torch.randn(2, 8, 48)  # (batch, seq, obs_dim)
    ...
    def _test_history_wrapper(self):
        ...
        assert obs.size == 8 * 48, f"Expected 384 elements, got {obs.size}"
        obs2, _, _, _, _ = wrapped.step(np.zeros(12, dtype=np.float32))
        assert obs2.size == 8 * 48, f"Expected 384 elements, got {obs2.size}"
        wrapped.close()
...
<pre>
Project: Mini Cheetah Locomotion
Base Obs Dim: 48   |  Terrain Obs Dim: 57
Action Dim: 12     |  Control: PD (kp=80, kd=1)
